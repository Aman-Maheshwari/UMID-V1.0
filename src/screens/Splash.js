...
import React, { Component } from 'react';
import { Text, View, Image, TextInput, TouchableOpacity, Modal, StyleSheet,ToastAndroid,AsyncStorage,Alert,Linking } from 'react-native';
import { heightPercentageToDP as hp, widthPercentageToDP as wp } from 'react-native-responsive-screen'
import * as firebase from 'firebase';
import { connect } from 'react-redux';
import CryptoJS from "react-native-crypto-js";
...
class Splash extends React.Component{
    ...
    componentDidMount(){
        setTimeout(() => {
          if(this.state.versionCheck == true){
            if(this.state.shouldLogin){
                this.props.navigation.navigate('Home' , {phone : this.state.phone , password : this.state.password })
            }
            else{              
            ...
            Alert.alert(
              "Update the App",
              `
This is an older version of the app, please update the app.
              `,
              [
                { text: "Update App", onPress: () => Linking.openURL("https://play.google.com/store/apps/details?id=com.umid") }
              ],
              { cancelable: false }
            );
          }
    ...
    body: JSON.stringify({'version': 9})}).then(res => {
      return res.json()
    }).then(res => {
