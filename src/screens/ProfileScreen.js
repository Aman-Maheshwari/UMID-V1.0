    ...
    initialData = () =>{
        console.log(this.props.phonenumberuser);
            ...
            User_details.then((data) => {
                ...
                this.props.dispatch({ type: 'UPDATE_TEXT', text: this.state.name })
    ...
    renderOrganizationActive = () => {
        ...
        return (
            <View >
                ...
                {/* <View style={{ flex: 0.38 }}> */}
                {this.state.showFlatList ?
                    <View style={{ marginLeft:wp('7.5%'),maxHeight: hp('13%'),marginRight:wp('7.5%') }} >
                        <FlatList
                            keyboardShouldPersistTaps={'always'}
                            data={this.state.dataSource}
                            ItemSeparatorComponent={this.ListViewItemSeparator}
                    ...
                    <View style={styles.backgroundContainerUpper}>
                        <View style={{ marginLeft: wp("4%"), paddingTop: hp("2.5%") }}>
                            <TouchableOpacity onPress={
                                () => {
                                    this.initialData()
                                    if(this.state.name.length !=0 && this.state.organisation.length !=0){
                                    this.setState({
                                        isEditable: false
                                    })}
                                }
